    ...
    visit(node.elts[0], options, function (err1, name) {
      visit(node.elts[1], options, function (err2, value) {
        ...
        visit(node.elts[2], options, function (err3, color) {
          var colorRegx = RegExp('^#(?:[0-9a-fA-F]{3}){1,2}$');
    ...
    visit(node.elts[0], options, function (err1, name) {
      visit(node.elts[1], options, function (err2, slices) {
        visit(node.elts[2], options, function (err3, pieOptions) {
          var _pieOptions$height = pieOptions.height,
              height = _pieOptions$height === undefined ? 300 : _pieOptions$height,
              _pieOptions$width = pieOptions.width,
              width = _pieOptions$width === undefined ? 400 : _pieOptions$width,
              _pieOptions$donut = pieOptions.donut,
              donut = _pieOptions$donut === undefined ? false : _pieOptions$donut;

          console.log('pie options', height, width, donut, pieOptions);
          height = +height;
          if (isNaN(height)) {
            err3 = err3.concat(error("Height must be a number.", node.elts[3]));
          }
          width = +width;
          if (isNaN(width)) {
            err3 = err3.concat(error("Width must be a number.", node.elts[3]));
          }
          donut = Boolean(donut);
          resume([].concat(err1, err2, err3), {
            type: "pie",
